...
export default function EntityTypesPage() {
  ...
  const load = () => fetch('/api/platform/service-master/entity-types').then(r => r.json()).then(setItems);
  useEffect(() => { load(); }, []);

  const openNew = () => { setForm({ name: '', description: '', sort_order: 0 }); setEditItem(null); setShowForm(true); };
  const openEdit = (item: any) => { setForm({ name: item.name, description: item.description || '', sort_order: item.sort_order || 0 }); setEditItem(item); setShowForm(true); };

  const save = async () => {
    const url = editItem ? `/api/platform/service-master/entity-types/${editItem.id}` : '/api/platform/service-master/entity-types';
  ...
  const del = async (id: string) => {
    ...
    await fetch(`/api/platform/service-master/entity-types/${id}`, { method: 'DELETE' }); load();
  };

  return (
    <div style={{ maxWidth: 800 }}>
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1>🏛️ Entity Types</h1>
        <button onClick={openNew} style={btn}><Plus size={16} /> Add Entity Type</button>
      </div>
      {showForm && (
        <div className="card" style={{ padding: 20, marginBottom: 16, border: '2px solid #4f46e5' }}>
          <h3 style={{ marginBottom: 12 }}>{editItem ? 'Edit' : 'Add'} Entity Type</h3>
