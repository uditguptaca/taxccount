...
export async function PATCH(req: Request) {
  try {
    ...
    if (task_type === 'stage') {
      ...
      if (action === 'start') {
        await db.prepare(`UPDATE staff_tasks SET status = 'in_progress', updated_at = ? WHERE id = ? AND org_id = ?`).run(now, task_id, orgId);
      } else if (action === 'complete') {
        await db.prepare(`UPDATE staff_tasks SET status = 'completed', completed_at = ?, notes = COALESCE(?, notes), updated_at = ? WHERE id = ? AND org_id = ?`).run(now, notes, now, task_id, orgId);
      } else if (action === 'add_note') {
        await db.prepare(`UPDATE staff_tasks SET notes = ?, updated_at = ? WHERE id = ? AND org_id = ?`).run(notes, now, task_id, orgId);
      } else if (action === 'cancel') {
        await db.prepare(`UPDATE staff_tasks SET status = 'cancelled', updated_at = ? WHERE id = ? AND org_id = ?`).run(now, task_id, orgId);
      }
    }
