...
export async function GET() {
  try {
    ...
    const result = await Promise.all((rows as any[]).map(async h => {
...
export async function POST(req: Request) {
  try {
    ...
    await getDb().prepare(`INSERT INTO sm_compliance_heads (id,name,short_name,description,icon,color_code,is_active,sort_order,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)`).run(
