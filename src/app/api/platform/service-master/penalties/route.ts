...
export async function POST(req: Request) {
  try {
    ...
    const id = uuidv4();
    await getDb().prepare(`INSERT INTO sm_penalties (id,sub_compliance_id,description,penalty_type,amount,rate,max_amount,details,is_active,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)`).run(
