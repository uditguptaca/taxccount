...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json(subCompId ? await db.prepare(sql).all(subCompId) : await db.prepare(sql).all());
...
export async function POST(req: Request) {
  try {
    ...
    const id = uuidv4();
    await getDb().prepare(`INSERT INTO sm_info_fields (id,sub_compliance_id,field_label,field_type,is_required,placeholder,help_text,options,sort_order,is_active,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)`).run(
