...
export async function POST(request: Request) {
  try {
    ...
    const id = uuidv4();
    await db.prepare(`INSERT INTO personal_entities (id, user_id, name, entity_type, registration_number, description, status, created_at, updated_at) VALUES (?,?,?,?,?,?,'active',?,?)`).run(id, userId, name, entity_type, registration_number, description, now, now);
...
export async function PATCH(request: Request) {
  try {
    ...
    if (body.compliance_id) {
      ...
      await db.prepare(`UPDATE personal_entity_compliance SET status = ?, urgency = ?, completed_at = ?, title = COALESCE(?, title), due_date = COALESCE(?, due_date), updated_at = NOW() WHERE id = ? AND user_id = ?`).run(newStatus, urgency, completedAt, title, due_date, compliance_id, userId);
      return NextResponse.json({ message: 'Updated' });
    }

    const { id, name, entity_type, registration_number, description, status } = body;
    if (!id) return NextResponse.json({ error: 'ID required' }, { status: 400 });
    await db.prepare(`UPDATE personal_entities SET name = COALESCE(?, name), entity_type = COALESCE(?, entity_type), registration_number = COALESCE(?, registration_number), description = COALESCE(?, description), status = COALESCE(?, status), updated_at = NOW() WHERE id = ? AND user_id = ?`).run(name, entity_type, registration_number, description, status, id, userId);
    return NextResponse.json({ message: 'Updated' });
  } catch (error) {
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'ID required' }, { status: 400 });
    if (type === 'compliance') {
      await db.prepare(`DELETE FROM personal_entity_compliance WHERE id = ? AND user_id = ?`).run(id, userId);
    } else {
      await db.prepare(`DELETE FROM personal_entities WHERE id = ? AND user_id = ?`).run(id, userId);
    }
