...
export async function GET() {
  try {
    ...
    const db = getDb();
    const members = await db.prepare(`SELECT * FROM personal_family_members WHERE user_id = ? ORDER BY relationship, name`).all(userId) as any[];
    for (const m of members) {
      m.compliance = await db.prepare(`SELECT * FROM personal_family_compliance WHERE family_member_id = ? AND user_id = ? ORDER BY due_date ASC`).all(m.id, userId);
    }
...
export async function POST(request: Request) {
  try {
    ...
    const id = uuidv4();
    await db.prepare(`INSERT INTO personal_family_members (id, user_id, name, relationship, date_of_birth, email, phone, notes, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)`).run(id, userId, name, relationship, date_of_birth, email, phone, notes, now, now);
...
export async function PATCH(request: Request) {
  try {
    ...
    if (body.compliance_id) {
      ...
      await db.prepare(`UPDATE personal_family_compliance SET status = ?, urgency = ?, completed_at = ?, title = COALESCE(?, title), due_date = COALESCE(?, due_date), updated_at = NOW() WHERE id = ? AND user_id = ?`).run(newStatus, urgency, completedAt, title, due_date, compliance_id, userId);
      return NextResponse.json({ message: 'Updated' });
    }

    const { id, name, relationship, date_of_birth, email, phone, notes } = body;
    if (!id) return NextResponse.json({ error: 'ID required' }, { status: 400 });
    await db.prepare(`UPDATE personal_family_members SET name = COALESCE(?, name), relationship = COALESCE(?, relationship), date_of_birth = COALESCE(?, date_of_birth), email = COALESCE(?, email), phone = COALESCE(?, phone), notes = COALESCE(?, notes), updated_at = NOW() WHERE id = ? AND user_id = ?`).run(name, relationship, date_of_birth, email, phone, notes, id, userId);
    return NextResponse.json({ message: 'Updated' });
  } catch (error) {
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: 'ID required' }, { status: 400 });
    if (type === 'compliance') {
      await db.prepare(`DELETE FROM personal_family_compliance WHERE id = ? AND user_id = ?`).run(id, userId);
    } else {
      await db.prepare(`DELETE FROM personal_family_members WHERE id = ? AND user_id = ?`).run(id, userId);
    }
