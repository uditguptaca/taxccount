...
export async function GET(req: Request) {
  try {
    ...
    const compliances = await db.prepare(`
    ...
    const docRequests = await db.prepare(`
    ...
    // ── UNREAD CHATS ──
    const unreadChats = await db.prepare(`
    ...
    try {linkedEntities = await db.prepare(`
    ...
    for (const le of linkedEntities) {
      ...
      const linkedComps = await db.prepare(`
        SELECT cc.*, ct.name as template_name, ct.code as template_code,
    ...
    try {docSummary = await db.prepare(`
        SELECT 
          COUNT(*) as total_docs,
    ...
    try {personalInfo = await db.prepare(`
    ...
    try {
      ...
      const vaultPersonal = await db.prepare(`
      ...
      const vaultFamily = await db.prepare(`
      ...
      const vaultEntities = await db.prepare(`
      ...
      // Consultants summary
      consultantSummary = await db.prepare(`
        SELECT pc.*, 
