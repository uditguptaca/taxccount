...
import { Inbox, FileText, DollarSign, CheckSquare, MessageSquare, AlertTriangle, ClipboardCheck, Archive, Bell } from 'lucide-react';
...
export default function StaffInboxPage() {
  ...
  if (loading) return <div style={{ padding: 'var(--space-8)', color: 'var(--color-gray-400)', textAlign: 'center' }}>Loading inbox...</div>;
  if (!data) return <div style={{ padding: 'var(--space-8)', color: 'var(--color-danger)', textAlign: 'center' }}>Failed to load inbox.</div>;
  ...
  return (
    <>
      <div className="page-header">
        <div>
          <h1 style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}><Inbox size={28} /> Inbox</h1>
          <p className="text-muted text-sm" style={{ marginTop: 'var(--space-1)' }}>Notifications and action items — {data.unreadCount || 0} unread</p>
        </div>
      </div>
      ...
      <div className="card">
        {items.length === 0 ? (
          ...
          items.map((item: any) => {
            const config = typeConfig[item.item_type] || { icon: Bell, iconClass: 'document' };
