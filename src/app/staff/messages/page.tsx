...
function formatTime(d: string) { return new Date(d).toLocaleTimeString('en-CA', { hour: '2-digit', minute: '2-digit' }); }
...
export default function StaffMessagesPage() {
  ...
  async function handleCreateThread(e: React.FormEvent) {
    ...
    if (data.success) {
      ...
      fetch('/api/messages').then(r => r.json()).then(d => { setThreads(d.threads || []); setSelectedThread(data.thread_id); });
    }
  }
  ...
  return (
    <>
      <div className="page-header">
        <div><h1 style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}><MessageSquare size={28} /> Messages</h1><p className="text-muted text-sm" style={{ marginTop: 'var(--space-1)' }}>Client and internal team communication</p></div>
      ...
      <div className="two-panel">
        ...
        <div className="two-panel-sidebar">
          <div className="two-panel-sidebar-list">
            ...
            {filteredThreads.length === 0 && <div className="text-center text-muted" style={{ padding: 'var(--space-6)' }}>No conversations yet</div>}
          </div>
        </div>
        ...
        <div className="two-panel-main">
          {threadData ? (
            <>
              ...
              <div className="chat-messages">
                {(threadData.messages || []).map((m: any, i: number) => (
                  <div key={i} className={`chat-bubble ${m.sender_role !== 'client' ? 'own' : 'other'} ${m.is_internal || threadData.thread?.thread_type === 'internal' ? 'internal-chat-bubble' : ''}`}
                    style={m.is_internal && threadData.thread?.thread_type !== 'internal' ? { backgroundColor: 'var(--color-yellow-50)', borderColor: 'var(--color-yellow-200)' } : {}}>
              ...
              <form className="chat-input-bar" onSubmit={handleSendMessage} style={{ display: 'flex', gap: 'var(--space-2)' }}>
                <div className="chat-input-toolbar">
                  <button type="button" title="Bold" onClick={() => setNewMessage(m => m + '**bold** ')}><Bold size={16} /></button>
                  <button type="button" title="Italic" onClick={() => setNewMessage(m => m + '_italic_ ')}><Italic size={16} /></button>
            ...
            <div className="empty-state"><MessageSquare size={48} /><h3>Select a conversation</h3><p>Choose a chat thread from the sidebar</p></div>
          )}
        </div>
      </div>
            ...
            <form onSubmit={handleCreateThread}>
              <div className="modal-body">
                ...
                <div className="form-group">
                  ...
                  <textarea className="form-input" required value={newThreadForm.message} onChange={e => setNewThreadForm({...newThreadForm, message: e.target.value})} placeholder="Type your first message..." style={{ minHeight: 100 }}></textarea>
                </div>
              </div>
