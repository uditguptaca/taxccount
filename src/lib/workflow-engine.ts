...
export async function triggerWorkflowEvent(
  ...
  try {
    ...
    const matchingRules = await db.prepare(query).all(...queryParams) as any[];
    ...
    for (const rule of matchingRules) {
      ...
      if (rule.conditions) {
        try {
          ...
          if (payload.client_id) {
            evalContext.client = await db.prepare('SELECT * FROM clients WHERE id = ? AND org_id = ?').get(payload.client_id, payload.org_id);
          }
      ...
      const actions = await db.prepare(`SELECT * FROM workflow_actions WHERE rule_id = ? AND org_id = ? ORDER BY sequence_order ASC`).all(rule.id, payload.org_id) as any[];
  ...
  switch (action_type as ActionType) {
    case 'MOVE_STAGE': {
      ...
      if (context.engagement_id && parsedPayload.target_stage_code) {
        ...
        if (stage) {
          ...
          await db.prepare(`UPDATE client_compliance_stages SET status = 'in_progress', started_at = ?, updated_at = ? WHERE id = ?`)
    ...
    case 'CREATE_TASK': {
      ...
      if (parsedPayload.assign_to === 'manager') {
        ...
        const engagement = await db.prepare('SELECT assigned_team_id FROM client_compliances WHERE id = ?').get(context.engagement_id) as any;
        if (engagement?.assigned_team_id) {
          const manager = await db.prepare('SELECT manager_id FROM teams WHERE id = ?').get(engagement.assigned_team_id) as any;
          if (manager?.manager_id) assigneeId = manager.manager_id;
        }
      } else if (parsedPayload.assign_to === 'current_assignee') {
        const currentStage = await db.prepare(`SELECT assigned_user_id FROM client_compliance_stages WHERE engagement_id = ? AND status = 'in_progress' LIMIT 1`).get(context.engagement_id) as any;
        if (currentStage?.assigned_user_id) assigneeId = currentStage.assigned_user_id;
      }

      // Fallback: assign to org admin
      if (!assigneeId) {
        const admin = await db.prepare(`SELECT user_id FROM organization_memberships WHERE org_id = ? AND role = 'firm_admin' LIMIT 1`).get(context.org_id) as any;
      ...
      await db.prepare(`
        ...
        VALUES (?, ?, ?, 'task_assigned', ?, ?, ?, 'staff_task', ?, 0, NOW())
      `).run(
    ...
    case 'SEND_MESSAGE': {
      ...
      if (!thread) {
        ...
        const client = await db.prepare('SELECT display_name FROM clients WHERE id = ?').get(context.client_id) as any;
      ...
      let senderId = context.actor_id;
      if (!senderId) {
        const adminRow = await db.prepare(`SELECT user_id FROM organization_memberships WHERE org_id = ? AND role = 'firm_admin' LIMIT 1`).get(context.org_id) as any;
        senderId = adminRow?.user_id || 'SYSTEM';
      }
      ...
      await db.prepare(`UPDATE chat_threads SET last_message_at = ?, updated_at = ? WHERE id = ?`)
    ...
    case 'AUTO_TAG': {
      ...
      let tag = await db.prepare(`SELECT id FROM client_tags WHERE org_id = ? AND name = ?`).get(context.org_id, tagName) as any;
