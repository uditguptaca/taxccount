const fs = require('fs');
let lines = fs.readFileSync('src/app/dashboard/settings/page.tsx', 'utf8').split('\\n');

let startIdx = lines.findIndex(l => l.includes('onClick={() => handleDisconnectIntegration(int.id)}>Disconnect</button>'));
let endIdx = lines.findIndex(l => l.includes('{activeTab === \\'currency\\' && ('));

if (startIdx !== -1 && endIdx !== -1) {
  // Replace everything between startIdx and endIdx with the proper integrations closing
  const replacement = [
  ...
  // Now we append the rate modal before the final </>
  ...
  console.log('Successfully fixed using slice/splice!');
} else {
