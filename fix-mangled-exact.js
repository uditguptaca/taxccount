...
const brokenBlock = `                              <button className="btn btn-secondary btn-sm" style={{ color: 'var(--color-danger)' }} onClick={() => handleDisconnectIntegration(int.id)}>Disconnect</button>
...
const correctIntegrationsEnd = `                              <button className="btn btn-secondary btn-sm" style={{ color: 'var(--color-danger)' }} onClick={() => handleDisconnectIntegration(int.id)}>Disconnect</button>
                            </>
                          ) : (
                            <button className="btn btn-primary btn-sm" style={{ flex: 1, background: int.color, borderColor: int.color }} onClick={() => openIntegrationConfig(int)}><Key size={13} /> Connect & Configure</button>
                          )}
...
if (code.includes(brokenBlock)) {
  code = code.replace(brokenBlock, correctIntegrationsEnd);
  ...
  console.log('Fixed broken block and added rate modal correctly!');
} else {
  console.log('Could not find broken block to replace!');
}
